...
final class TypeNodeHelper
{

	private const array UserSafeTypes = [
		'int' => true,
		'string' => true,
		'float' => true,
		'bool' => true,
	...
	private const array SimplifiedTypes = [
		'boolean' => 'bool',
		'true' => 'bool',
		'false' => 'bool',
	...
	private const array NativeKeywords = [
		'array' => true,
		'bool' => true,
		'callable' => true,
		'false' => true,
	...
	private const array Keywords = self::NativeKeywords + [
		'boolean' => true,
		'double' => true,
		'empty' => true,
		'integer' => true,
	...
	public static function isKeyword(IdentifierTypeNode $type): bool
	{
		return isset(self::Keywords[$type->name])
			   || isset(self::NativeKeywords[strtolower($type->name)])
	...
	): ?TypeNode
	{
		if ($reflection instanceof ReflectionFunctionAbstract) {
			$type = $reflection->getReturnType() ?? $reflection->getTentativeReturnType();
		} else {
	...
	public static function fromReflectionType(ReflectionType $reflectionType): TypeNode
	{
		if ($reflectionType instanceof ReflectionNamedType) {
			...
			$type->setAttribute('reflection', true);
	...
	public static function removeNullFromUnion(UnionTypeNode $node): ?TypeNode
	{
		$types = $node->types;
		$nullable = false;
	...
	public static function removeNull(TypeNode $type): ?TypeNode
	{
		if ($type instanceof IdentifierTypeNode && ($type->name === 'null' || $type->name === 'mixed')) {
			return null;
		}
		...
		if ($type instanceof UnionTypeNode) {
			return self::removeNullFromUnion($type);
		}

		return null;
	}

	public static function addNull(TypeNode $type): TypeNode
	{
	 ...
	 */
	public static function resolveClassKeywords(
		mixed $type,
	...
	): void
	{
		if (is_array($type)) {
			foreach ($type as $item) {
				self::resolveClassKeywords($item, $context, $genericParameterNames);
			}
		} elseif ($type instanceof IdentifierTypeNode) {
			if ($type->hasAttribute('reflection')) {
				return;
			}
			...
			self::resolveClassKeywords($type->valueType, $context, $genericParameterNames); // intentionally not resolving key type
	...
	private static function getGenericTypeDefinitionFromPhpDoc(string $className): GenericTypeDefinition
	{
		...
		foreach ($genericParameters as $genericParameter) {
			self::resolveClassKeywords($genericParameter->bound, $classReflection, array_keys($genericParameters));
			self::resolveClassKeywords($genericParameter->default, $classReflection, array_keys($genericParameters));
		}
		...
		foreach ($phpDocNode->children as $node) {
			if ($node instanceof PhpDocTagNode && ($node->value instanceof ImplementsTagValueNode || $node->value instanceof ExtendsTagValueNode)) {
				self::resolveClassKeywords($node->value->type, $classReflection, array_keys($genericParameters));
...
//		if ($item->keyName instanceof IdentifierTypeNode) {
//			return new ConstTypeNode(new ConstExprStringNode($item->keyName->name, ConstExprStringNode::SINGLE_QUOTED));
//		}

		throw new LogicException('Invalid array shape key');
	}

	public static function normalize(TypeNode $type): TypeNode
	{
		...
		if ($type instanceof UnionTypeNode) {
			...
			if ($nullable) {
				...
				if (count($list) > 1) {
					return new NullableTypeNode(new UnionTypeNode(array_values($types)));
				}

				return new NullableTypeNode($list[0] ?? new IdentifierTypeNode('mixed'));
			}
	...
	/**
	 * @return non-empty-list<'int'|'string'|'float'|'bool'|'array'|'null'>|null
	 */
	public static function getUserSafeTypes(TypeNode $type): ?array
	{
		$type = self::simplifyType($type);

		if ($type instanceof IdentifierTypeNode) {
			if (!isset(self::UserSafeTypes[$type->name])) {
				return null;
			}
		...
		if ($type instanceof UnionTypeNode) {
			...
			foreach ($type->types as $subType) {
				$ret = self::getUserSafeTypes($subType);
				if ($ret === null) {
					return null;
				}
	...
	public static function simplifyType(TypeNode $type): TypeNode
	{
		if ($type instanceof IdentifierTypeNode) {
			if (self::isKeyword($type)) {
				$name = strtolower($type->name);
				if (isset(self::SimplifiedTypes[$name])) {
					$types = self::SimplifiedTypes[$name];
					if (is_array($types)) {
						return new UnionTypeNode(array_map(fn (string $type): IdentifierTypeNode => new IdentifierTypeNode($type), $types));
					}

					return new IdentifierTypeNode($types);
				}

				return new IdentifierTypeNode($name);
			}
			if ($type->name === Generator::class) {
		...
		if ($type instanceof NullableTypeNode) {
			return new UnionTypeNode([self::simplifyType($type->type), new IdentifierTypeNode('null')]);
		}
		...
		if ($type instanceof GenericTypeNode) {
			return self::simplifyType($type->type);
		}
