 ...
 */
final readonly class NumberInclusiveRange implements NumberRange
{
	...
	{
		if ($this->exact) {
			return $this->min; // min and max are equal when exact is true
		}
	 ...
	 * @param TValue $value
	 */
	public function decide(int|float $value): RangeInclusiveDecision
	{
		if ($this->min !== null) {
			if (is_float($this->min)) {
				if (Floats::isLessThan($value, $this->min)) {
					return RangeInclusiveDecision::ShouldBeGreaterOrEqual;
				}
		...
		if ($this->max !== null) {
			if (is_float($this->max)) {
				if (Floats::isGreaterThan($value, $this->max)) {
					return RangeInclusiveDecision::ShouldBeLessOrEqual;
				}
		...
		return RangeInclusiveDecision::Ok;
	}
